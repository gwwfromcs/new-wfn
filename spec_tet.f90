...
subroutine spec_tet(vol, e, f, ngrid, ebound, edos, fgrid, neqtet)
  ...
  !
  !
  !     and the DOS from one tetrahedron. This is done for energies
  !     defined by ebound.
  !
  !
  !     ------------    local variables -----------------------
  ...
  do i = istart, iend                     !  loop over energy grid
     ...
     if (ei <= es(1)) then  
        ...
        s = dthree * x * x / dn1  
        fm = (f21 / d21 + f31 / d31 + f41 / d41) * x * dthird
        fms = fm * s  
        ...
        if (es(1) /= es(2)) then  
           s = dthree / dn1 * x * x - dthree / dn2 * (x - d21)**2  
           fms = (f21 / d21 + f31 / d31 + f41 / d41) * x**3 / dn1 - &
                (f21 / d21 + f32 / d32 + f42 / d42) * (x - d21)**3 / dn2 - &
                dthree * f21 * (x - d21)**2 / dn2
        else  

           s = dthree * vol * x / (d41 * d31) * (dtwo - x * (done / d31 + &
                done / d41))
           fms = vol * x / (d41 * d31) * (dthree * f21 + dthree * x * &
        ...
        s = dthree * (x - d41)**2 / dn4  
        fm = (f41 / d41 + f42 / d42 + f43 / d43) * (x - d41) * dthird + &
             f41
        fms = fm * s  
