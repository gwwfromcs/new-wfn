!
subroutine writegsdat(ipr, gs, data, ldnvecs, nvecs, nspin, filename, namelen)
  ...
  complex(dp), intent(in) :: &
       data(gs%length, ldnvecs, nspin)  ! data that should be written
  character(len=*), intent(in) :: &
       filename              ! name of file to be written
  !
  !     ---------------- local variables ---------------------------------
  !
  integer :: i, j, k, iproc, wios, n, is  
  ...
  if (gs%myproc == 0) then  
     if (gs%nproc > 1) then  
        open(unit = 21, file = buffer, position = 'append', status = 'unknown', &
             form = 'unformatted')
     end if
     ! mark end of record
