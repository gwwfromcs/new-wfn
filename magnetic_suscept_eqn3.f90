...
!   Using equation 3 of Gregor et al 1999
!
!   (1999)      Chris Pickard and Francesco Mauri
!               Paris/Kiel
! ----------------------------------------------------------------------
 
Subroutine magnetic_suscept_eqn3(qmag,g0mask,pw_params,ffts,bands,pot_gspace,&
  ...
  !     -------------- Local variables ---------------
  !
  Integer :: alpha,beta  ! Counters for directions of momentum operator
  ...
  Real(dp) :: RI(3)               ! temp position    


  Complex(dp) :: csum        ! Complex sum
  Real(dp) :: para_temp(3,3) ! Temp current array

  Complex(dp), Pointer :: nloc_deriv_k(:,:,:,:) ! Non-loc deriv at k
  ...
  Complex(dp), Pointer :: J_r(:,:,:)     ! J_magnetic in rspace J_r(r,a0,a1)
  Complex(dp), Pointer :: rho(:),rrho(:) ! Charge density   
  ...
  Real(dp), Allocatable :: para_corr_shift_temp(:,:,:,:)
  ...
  Write (9,*)
  Write (9,*) '***************************'
  Write (9,*) 'MOLECULAR ALGORITHM (Eqn 3)'
  Write (9,*) '***************************'
  ...
  Allocate(para_corr_shift_temp(magnet,current,crys%ntype,crys%mxdatm))
  ...
  !
  Allocate(J_r(ffts%r_size,3,3),rho(ffts%r_size),rrho(ffts%r_size))
  Allocate(work(ffts%r_size))
  J_r = zzero
  rho = zzero
  ...
  If(Iand(pw_params%input,4).Eq.4)  &
       Call chkpt_chi_mol(irk,J_r(1,1,1),rho,9*pot_gspace%r_size, &
       pot_gspace%myproc,pw_params%output(1),2)
...
20      Continue
     ...
     If(ierr.Eq.1) Then
        ...
        Call start_vectors(0,pw_params,ham,densham,bands%energy(1,irk,1),&
             bands%max,u_k(1,1),pot_gspace,ksub_gspace,vion%data(1,1,1),crys)
     ...
     If(pw_params%bandgap.Gt.0) Then
        ...
        bands%energy(bands%min(1)+1,irk,1)=bands%energy(bands%min(1),irk,1)+&
             pw_params%bandgap
     Endif
     ...
     !     Allocate memory for u_k~

     Allocate(u_ktil(k_gspace%length,bands%ifmax(irk,1)))

     !
     !     Calculate part of the current correction
     !
     ...
     Do beta=1,3
        ...
        Do i=1, bands%ifmax(irk,1)
           ...
           Call apply_r2psi(beta,k_gspace,ffts,rinr(1,1),u_k(1,i),u_ktil(1,i)) 

        End Do

        !     Evaluate <psi|r_beta delta_J|rpsi>-<psi|delta_J r_beta|rpsi>

        Call paramagnetic_correction_mol(para_corr_shift,ham,crys, &
             blop,u_ktil(1,1),u_k(1,1),bands%ifmax(irk,1),(0.d0,-1.d0))

        para_corr_shift_temp(beta,:,:,:) = para_corr_shift

        Call paramagnetic_correction_mol(para_corr_shift,ham,crys, &
             blop,u_k(1,1),u_ktil(1,1),bands%ifmax(irk,1),(0.d0,-1.d0))

        para_corr_shift_temp(beta,:,:,:) = &
             para_corr_shift_temp(beta,:,:,:)-para_corr_shift

     End Do

     !     But we want r.(R_IxB) for B in beta direction

     Do nt=1,crys%ntype
        Do iat=1,crys%natom(nt)

           RI = Matmul(crys%avec,crys%rat(:,iat,nt))/pi2
           para_temp = para_corr_shift_temp(:,:,nt,iat)
           para_corr_shift_temp(1,:,nt,iat) = &
                para_temp(:,2)*RI(3)-para_temp(:,3)*RI(2)
           para_corr_shift_temp(2,:,nt,iat) = &
                para_temp(:,3)*RI(1)-para_temp(:,1)*RI(3)
           para_corr_shift_temp(3,:,nt,iat) = &
                para_temp(:,1)*RI(2)-para_temp(:,2)*RI(1)
        End Do
     End Do

     !     Don't forget the factor of 1/(4i) ?where does the extra 2 come from?
     !      - and the k-point weighting

     para_corr_shift_tot = para_corr_shift_tot+para_corr_shift_temp*kpoints%w(irk)/4.d0     
     ...
     Do beta =1,3            
        !     
        !     Compute [(r-d) x p + DVnl_I x R_I]_beta|u_k>
        !     
        ...
        Do i=1, bands%ifmax(irk,1)
           ...
           !     First [(r-d) x p]_beta|u_k>

           Do j=1,-1,-2
              Call apply_mom(indx(j,beta),k_gspace,rk(1),u_k(1,i),phi(1),crys)
        ...
        Call cg_blocksolve_mol(pw_params%output(1),crys,ham,&
             pw_params%maxitcgsol,pw_params%epsmag,&
             bands%ifmax(irk,1),bands%energy(1,irk,1),&
             u_k(1,1),u_ktil(1,1),graduk(1,1),pw_params%nbandsfft)
        ...
        Do i=1,bands%ifmax(irk,1) ! Loop over occupied bands
           ...
           If(beta.Eq.1) Call add_to_j_bare(ffts,k_gspace,u_k_rspace(1),&
                u_k(1,i),kpoints%w(irk),rho(1))
     ...
     Deallocate(u_k,u_ktil,phi,phi1,nloc_deriv_k)
  ...
  !     
  !     ------ Now add the charge contribution to J_r ------
  !

  J_r = -2.0d0*J_r ! Factor of two for Hartree <--> Rydberg

  Do beta=1,3
     Do alpha=1,3
        Call apply_r2rho(indx2(beta,alpha),ffts,rinr(1,1),rho(1),rrho(1))
        J_r(:,beta,alpha) = J_r(:,beta,alpha)-rrho(:)
     End Do
  End Do
  ...
  Deallocate(vloc,J_r,rho,rrho,rinr,dia_corr_shift,dia_corr_shift_tot)
  Deallocate(para_corr_shift_tot,para_corr_shift_temp,para_corr_shift,work)
  Deallocate(chi_g)
...
End Subroutine magnetic_suscept_eqn3
