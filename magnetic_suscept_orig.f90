...
!     @process extchk
!
subroutine magnetic_suscept_orig(qmag, g0mask, pw_params, ffts, bands, &
     pot_gspace, vion, crys, syms, pspot, kpoints, chi_g)
  ...
  !     ------
  !
  complex(dp), intent(out) :: chi_g(pot_gspace%length, 3, 3)  
  !
  !
  !     DESCRIPTION:
  !     -----------
  ...
  integer :: p0, p1, alf, &  ! counters for directions of momentum operator
       ...
       irk                   ! kpoint counter
  integer, parameter :: dq(3, 3) = reshape(source=(/ 1, 0, 0, 0, &
       1, 0, 0, 0, 1 /),shape=(/3,3/))  
...
!                u_kq(1, 1))
           ...
           call diagonalize_hamiltonian(time,1,num_HPsi,pw_params, pw_params%maxitdiag, &
                idiaconv, bands%min(1), n_ukq, pw_params%epsdiag, e_kq(1), ham, &
                u_kq(1, 1))
           ...
           call setup_nonlocal_derivative(qmag, kq(1), &
...
end subroutine magnetic_suscept_orig
