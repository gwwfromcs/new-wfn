...
! davegp
  allocate(rho_band(m)); rho_band=dzero
! davegp
...
!    rho = dble(parallel_zdotc(len*m,gp(1,1),1,g(1,1),1)) 
    do i=1,m
      rho_band(i) = dble(parallel_zdotc(len,gp(1,i),1,g(1,i),1))
    enddo
    err = maxval(abs(rho_band))
    rho = sum( rho_band )
! davegp
...
!       write(9,*) 'gmcg iter #',n+nover_cnt,rho,energy1
    if (iand(pw_params%output(1), 536870912 ) == 536870912 ) &
       write(9,*) 'gmcg iter #',n+nover_cnt,' err=',err,energy1
! davegp
    call myflush(9)
    ...
    oldrho = rho  
! davegp
! modified to check for abs max error over bands
!    if (rho < delta ) then  
    if (err < delta ) then  
        iconv = .true.  
! davegp
...
!        iconv = .true.  
    ...
    if (n.eq.maxit) write(9, 991) n, m, rho, ham%shift  
...
! davegp
! if converged print out the error per band
!  if(iconv) then

! 
!    write(9,'(2x,a,e20.10)') 'abs max error = ', err
!    write(9,'(2x,a)') 'band convergence:'
!    do i=1,m
!      write(9,'(2x,"band ",i10,2x,"err = ",e20.10)') i, rho_band(i)
!    enddo
...
! davegp
! remove the redifinition of neig to be only occupied bands
