...
!
subroutine zdiag_scalapack(context, neig_req, n, nb, nprow, npcol, a, nc, nr, w)
  !
  ...
  integer, intent(in) :: &
       ...
       neig_req, &              ! number of eigenvalues sought for
       n, &                 ! matrix size
       nb, &                ! block size
  ...
  neig = min( neig_req, n ) ! can have at most n eigvals
  if( neig==0 ) neig=n
...
end subroutine zdiag_scalapack
